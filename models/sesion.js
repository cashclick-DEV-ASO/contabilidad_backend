...
const qryActualizaSesion = "UPDATE sesion SET vencimiento = DATE_ADD(NOW(), INTERVAL 30 MINUTE) WHERE token = ?"
...
export class SesionModel extends Modelo {
    ...
    async validaSesion(token) {
        if (!token) return this.sinDatos("La función validaSesion de la clase SesionModel no recibió el token.")
        ...
        return this.responde({ mensaje: this.mensaje }, this.error, false)
    }
}
