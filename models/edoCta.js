    ...
    static async insertaTransacciones(datos) {
        if (!datos)
            return this.sinDatos(
                "La función insertaTransacciones de la clase EdoCtaModel no recibió datos."
            )

        this.mensaje = "Transacciones agregadas correctamente."

        try {
            this.conexion = await this.db.getConnection()
            await this.conexion.beginTransaction()
            const { periodo, archivo, idCta } = datos
            const [resultadoEdoCta] = await this.conexion.query(qryEdoCta, [
                periodo,
                archivo,
                idCta
            ])
            if (resultadoEdoCta.affectedRows == 0)
                throw new Error("No se logro insertar el estado de cuenta.")
            const idArchivo = resultadoEdoCta.insertId
            const valores = datos.movimientos.map((movimiento) => {
                return [
                    idArchivo,
                    movimiento.linea,
                    movimiento.informacion,
                    this.fechaMySQL(movimiento.fechaCreacion),
                    this.fechaMySQL(movimiento.fechaValor),
                    movimiento.concepto,
                    movimiento.tipo,
                    movimiento.monto,
                    movimiento.idLayout
            ...
            if (resultadoMov.affectedRows == 0)
                throw new Error("No se logro insertar las transacciones.")
